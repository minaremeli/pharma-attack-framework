import argparse
from attacks.configs import ModelConfig, TrunkActivationAttackConfig, NGMAttackConfig, LeavingAttackConfig, str2bool, MultiModelTrunkActivationAttackConfig
from attacks import NGMAttack, TrunkActivationAttack, BaseAttack, LeavingAttack, ActiveTrunkActivationAttack, MultiModelTrunkActivationAttack
import torch
import random
import numpy as np
import os
...
class AttackLauncher():
    def __init__(self):
        ...
        parser.add_argument("--attack_name", help="The name of the attack you want to launch.", required=True, type=str,
                            choices=["NGMA", "TrunkActivation", "NOATTACK", "Leaving", "ActiveTrunkActivation", "MMTrunkActivation"])
        ...
        if self.attack_name == "NOATTACK":
            ...
            elif self.attack_name == "MMTrunkActivation":
                attack_config = MultiModelTrunkActivationAttackConfig(attack_config_parser)
                attack = MultiModelTrunkActivationAttack(attack_config, run_config, results_path=self.results_file, load_saved_model=self.load_saved_model, model_save=self.model_save, save_path=self.model_save_path)
